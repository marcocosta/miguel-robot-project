...
def clean_text_for_speech(text: str) -> str:
    ...
    # Remove common Markdown formatting that sounds bad in TTS.
    text = text.replace("**", "")
    ...
    # Replace bullets and separators with pauses.
    text = text.replace("•", ". ")
    text = text.replace("—", ", ")
    text = text.replace("–", ", ")
    text = text.replace("|", ", ")

    # Remove markdown links but keep readable text.
    text = re.sub(r"\[([^\]]+)\]\([^)]+\)", r"\1", text)

    # Remove emojis / unusual symbols outside normal speech range.
    text = re.sub(r"[^\w\s.,?!'\":;()/%-]", "", text)

    # Avoid TTS reading repeated punctuation strangely.
...
def speak(text: str):
    ...
    subprocess.run(["espeak", "-w", str(wav_path), speech_text], check=True)
