...
class SemanticSafetyClassifier:
    ...
    def classify(self, user_text: str, recent_context: str = "") -> SemanticSafetyResult:
        ...
        if self._is_fast_local_allow(user_text):
            return SemanticSafetyResult(
                decision="ALLOW",
                category="normal_conversation",
                confidence=0.97,
                reason="User is making normal conversation, family/project talk, or fictional story discussion.",
            )
        ...
        # Fast allow for normal Miguel development/planning talk.
        # This avoids slow semantic calls for harmless robot-improvement ideas.
        if self._is_robot_development_planning(user_text):
            return SemanticSafetyResult(
                decision="ALLOW",
                category="robot_project_debugging",
                confidence=0.97,
                reason="User is discussing normal Miguel robot development or improvement planning.",
            )
...
"""
    ...
    def _is_obviously_safe_smalltalk(self, text: str) -> bool:
        ...
        # "hello, miguel, how are you?" -> "hello miguel how are you"
        ...
        return any(t.startswith(p) for p in safe_starts)


    def _is_fast_local_allow(self, text: str) -> bool:
        raw = str(text or "").lower().strip()
        t = re.sub(r"[^a-z0-9\s]", " ", raw)
        t = re.sub(r"\s+", " ", t).strip()
        ...
        if self._contains_direct_harmful_request(t):
            return False
        ...
        if any(marker in t for marker in direct_unsafe_markers):
            return False

        if t in {"yes", "yeah", "yep", "sure", "ok", "okay", "nice", "good", "no", "nope", "not now", "maybe"}:
            return True
        ...
        if any(t.startswith(s) for s in conversation_starts):
            return True
        ...
        return any(p in t for p in safe_phrases)
    ...
    def _is_robot_development_planning(self, text: str) -> bool:
        t = text.lower()

        if self._contains_robot_planning_risky_marker(t):
            return False
        ...
        patterns = [
            r"\bimprove\b.*\bmiguel\b",
            r"\bmiguel\b.*\bimprove\b",
            r"\bimprove\b.*\brobot\b",
            r"\brobot\b.*\bimprove\b",
            r"\bimprove\b.*\bvoice\b",
            r"\bimprove\b.*\bcamera\b",
            r"\bimprove\b.*\bface\b",
        ]

        return any(re.search(p, t) for p in patterns)

    def _contains_robot_planning_risky_marker(self, text: str) -> bool:
        t = text.lower()
