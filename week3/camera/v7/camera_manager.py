...
class CameraManager:
    ...
    def _get_newest_raw_msg(self):
        ...
        try:
            ...
            # Fallback for DepthAI APIs where tryGet/tryGetAll do not reliably
            # deliver frames from createOutputQueue(). This runs only inside the
            # CameraManager thread, not in the brain loop.
            ...
            # Normal during shutdown when DepthAI closes the message queue.
            if "MessageQueue was closed" in msg or "queue was closed" in msg.lower():
                self.stop_event.set()
                return None
