...
- Never claim the camera sees a face/person unless local_state.face_detected is true and local_state.recognizer is not conversation_grace_cached_identity.
- For any request about seeing, looking, camera, vision, faces, or describing what is in front of you, do not answer from memory. Require fresh camera data.
...
def set_cached_local_state(new_state):
    with STATE_LOCK:
        ...
        # Identity hysteresis only applies to normal live camera frames.
...
def handle_user_turn_with_cached_state(user_text, cached_local_state):
    ...
    if mode_reply in {"__SHUTDOWN__", "__STOP_ROBOT__"}:
...
def flush_camera_queue(camera_queue, drain_frames=80):
    """
...
def get_state_for_user_turn(user_text, camera_queue, fallback_state):
    ...
    """
    try:
        if (not is_scene_description_request(user_text)) and is_any_camera_request(user_text):
...
def capture_latest_camera_msg(camera_queue, warmup_seconds=1.0, discard_after_wait=8):
    """
